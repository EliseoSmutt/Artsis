...
{
    public partial class NuevoAbonadoViewModel : ObservableObject
    {
        [ObservableProperty]
        ...
        public NuevoAbonadoViewModel(Popup popup, AbonadoApiService abonadoApiService)
        {
            _popup = popup;
        ...
        [RelayCommand]
        private async Task GuardarAbonado()
        {
           ...
           if(string.IsNullOrWhiteSpace(Telefono))
    {
            ...
            try
            {
                ...
                await _abonadoApiService.GuardarAbonado(abonado);
                await Application.Current.MainPage.DisplayAlert("Registrar abonad@", "Abonad@ registrado exitosamente!", "OK");
                _popup.Close();
            }
            catch (Exception ex) when (ex.Message.Contains("El DNI ingresado ya está registrado"))
            {
                await Application.Current.MainPage.DisplayAlert("Error", "El DNI ingresado ya pertenece a un abonado registrado. Por favor, verifique los datos e intente nuevamente.", "OK");
            }
            catch (Exception ex)
            {
